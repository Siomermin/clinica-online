...
export class MiPerfilComponent {
  ...
  ngOnInit() {
    this.authService.getUserData().subscribe((data) => {
      if (data) {
        ...
        this.horarioService.getHorarioPorEspecialista(this.usuarioLogeado.email).subscribe((data) => {
  ...
  updateTurno(entry: any): void {
    ...
    this.horarioService.updateHorario(this.horarios)
      .then(() => {
  ...
  getEspecialidad(day: string): string {
    const scheduleEntry = this.horarios?.horarios.find((entry: { dia: string }) => entry.dia === day);
    ...
    if (scheduleEntry) {
      ...
      if (currentIndex === -1 || currentIndex === especialidades.length - 1) {
        scheduleEntry.especialidad = especialidades.length > 0 ? especialidades[0] : '';
      } else {
  ...
  updateEspecialidadInDatabase(day: string, especialidad: string): void {
    ...
    if (scheduleEntryIndex !== -1) {
      updatedScheduleEntry.horarios[scheduleEntryIndex].especialidad = especialidad;

      // Call the updateHorario method in the HorarioService
      this.horarioService.updateHorario(updatedScheduleEntry)
        .then(() => {
